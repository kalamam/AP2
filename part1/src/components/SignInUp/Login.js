...
const signInSecretSuper = async(username, token, setUser) => {
...
const SignInSecret = ({username, token,setUser}) => {
    const navigate = useNavigate();
    signInSecretSuper(username, token, setUser).then(user => {
        // If valid
        if (user) {
            setUser(user);
            navigate("/");
...
const signInAgain = async (username, password,token) => {
...
const SignInFormAgain = ({setUser,token}) => {
    const usernameBox = useRef(null);
    const passwordBox = useRef(null);
    const navigate = useNavigate();
    ...
    const handleSignIn = async (e) => {
        ...
        signInAgain(username, password, token).then(user => {
            // If valid
            if (user) {
                setUser(user);
                navigate("/");
